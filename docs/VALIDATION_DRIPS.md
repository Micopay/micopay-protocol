...
**Contributor:** Anonymous · **Issue:** [#234](https://github.com/ericmt-98/micopay-protocol/issues/234) · **Region:** LATAM (South America)

First-person response (privacy-safe):

- **How clients/employers abroad pay me today:** The primary channel is a wire transfer routed through international correspondent banks, which eventually lands in a local USD or peso account after clearing multiple intermediary hops. For smaller or faster jobs I have used a Wise-style service that converts and deposits directly, and occasionally a client pays in USDC on-chain, which I then have to manually off-ramp through a P2P marketplace.

- **The worst part of receiving that money:** The combination of conversion rate opacity and unpredictability is the sharpest pain. By the time a wire clears — sometimes three to five business days — the official rate applied by the receiving bank is whatever it wants to use that day, not the rate I agreed to with the client or saw in the app when the transfer was initiated. Wise-style services are more transparent, but their local payout leg still relies on a bank deposit that can take an extra day. On-chain USDC is instant cross-border but the off-ramp step introduces counterparty risk and a spread that varies widely depending on the P2P order book depth at that moment. Account holds and compliance reviews are also a real risk: a large inflow can trigger a freeze while the bank asks for documentation, leaving the funds inaccessible for days.

- **Rough share lost to fees + spread combined:** Somewhere in the 4–8% range depending on the route and the day. Bank-to-bank is closer to the high end once correspondent fees and the conversion spread are both counted. Wise-type services sit in the 2–4% range. P2P crypto off-ramp varies the most — sometimes as low as 1–2% on a good day, sometimes 5%+ if liquidity is thin.

- **Would "dollars I can cash out locally or spend at a store" solve a real problem?** Yes — and the part that matters most is **certainty of the final local value**. Speed is nice, but the real problem is not knowing what I will actually pocket until the transfer settles. If I could receive payment in a dollar-denominated digital balance and then choose *when* to convert or cash out at a rate I can see upfront, that eliminates the two worst parts: the rate uncertainty and the bank-hold risk. Being able to walk to a nearby trusted agent and get local cash the same day I choose to convert would also remove the P2P counterparty step entirely.

- **What would make me not trust getting paid this way:** Any arrangement where the payer sends funds and I have no on-chain proof that the money is locked and earmarked for me before I acknowledge receipt. If the escrow is just a promise in an app without a verifiable transaction, I would not use it for anything beyond a trial. I would also walk away if the cash-out agent pool is thin or unrated — one bad experience with an agent who disappears after taking the transaction would be enough. And any sign of regulatory gray area that could trigger account freezes on the receiving side would be disqualifying.

**SDF narrative:** Adds LATAM freelancer/remote-worker inflow demand to Claim 7. The 4–8% total friction range, combined with rate uncertainty and hold risk, confirms that existing routes are expensive *and* unpredictable. The clearest value proposition is not just lower fees but **rate certainty at point of receipt** — a locked dollar balance the worker controls, cashable locally on their timeline. This is the lightweight on-ramp to full payroll (Vertical C) and directly validates the dollar-inflow demand signal.
